...
class Player(pygame.sprite.Sprite):
    def __init__(self, x, y, width, height, name):
        super().__init__()
        # Chill Boy Frames
        self.run_frames = [load_image(f'running-game-animations/chill boy/running/running-{i}.png') for i in range(1, 10)]
        self.run_frames = [pygame.transform.scale(frame, (width + 50, height + 60)) for frame in self.run_frames]
        self.idle_frames = [load_image(f'running-game-animations/chill boy/idle/frame-{i}.png') for i in range(11)]
        self.idle_frames = [pygame.transform.scale(frame, (width + 50, height + 60)) for frame in self.idle_frames]
        ...
        self.rect = self.image.get_rect(topleft=(x, y + 10))  # shift slightly down on load
